...
void CAN_LoadDefaultToEepRam(void)
{
	...
	//snake20161017 GPS的默认参数   默认定位信息为江西好帮手重庆办事处
...
#if 1
//snake 20160515
//返回0,写入成功;返回正数,写入失败(后期可以作为写入失败的错误代码)
...
#else  //snake2016116 解决不带网络管理功能的时候,ACC OFF,开启BATT,主机不进入睡眠的问题
	CANbus_DefaultInit(CAN_ID_BODY);		
	CANbus_StartupHandler(CAN_ID_BODY);
	//CANUser_StartSendInitMsg(CAN_ID_BODY);//会有CAN报文发出...
...
void CANTask_OtherMsgHandler(tMSG_BUF* Msg)
{
    ...
    case OP_OTHER_H:
        ...
        if(CanPMState.state != CAN_PM_UNNORMAL)
        {
            ...
            {
                //CANTask_SendPMReqest(OP_PM_STATE_WAIT_SLEEP);
		   PM_EnterMode(PM_MODE_OFF); //bolte 高压直接进待机模式
...
uint8_t CANTask_GetSysACCState(void)
{
    if(CANTask_BusConnectFlag())
    	{
   		 if(1 == CANTask_GetRemoteStartState())		// BOLTE:对策远程启动HU启动，远程启动要求HU保持睡眠
       		 return 0;
    	}
...
uint8_t CANTask_GetBusSleepState(void)
{
    ...
    #ifdef D_ILL_WAKEUP
    if((IllState ==1) && (IO_Get(IO_IDX_LOCAL_ACC,PIN_INPUT,TRUE)==0))// 小灯开的时候,关ACC  snake20160921
		return 2;
	#endif
	
    if(INH == 0)
    {
        /*自ACC OFF起时间超时后，才能够确认INH = 0;*/
        /*当前时间为6s, 自ACC OFF发送OP_PM_STATE_WAIT_SLEEP起设置
           超时时间。主要解决ACC OFF很快总线休眠，整机来不及
           记忆的问题。时间值可以在宏ACCOFF_TIMEOUT_S中设置*/
           /*//snake20161019
           //因为初始化设置为0,所以在有的时候,还没有来得及设置CanPMState.ACCOffTimer1S电源检测引脚就发生了变化,
           //导致,关机时间太短,没有按照延时的流程进行延迟关机...所以在将延时时间的判断标志设置为1
           */
...
void CANTask_PmRoutine(void)
{
...
#if 0		//在WAIT_SLEEP模式下，不进行总线休眠关机操作。2016-5-29 byzwl
        if(IOStatus == 0)
        {
            CANTask_SendPMReqest(OP_PM_STATE_OFF);
            CanPMState.state = CAN_PM_SLEEP;
    ...
    case CAN_PM_NORMAL:
	{
		CarAVMDataStruct *pData = pCAN_GetAVMData();
	        ...
	        else
	        {
				...
				{
					if(pData->CarAVMReq == 1) // 在全景模式关ACC，退出全景模式
					{
				...
				//需要增加在ILL开的情况下,开ACC的逻辑 snake20160816
	...
	case CAN_PM_WAIT_HALFSLEEP://开小灯的时候,关闭ACC的状态....进入并停留在WAIT_SLEEP模式...
		if(CanPMState.LocalACC) //开ACC,亮屏,开机
		{
			CANTask_SendPMReqest(OP_PM_STATE_ON);
			CanPMState.state = CAN_PM_NORMAL;
		}

		//if(IO_Get(IO_IDX_ILL_DET,PIN_INPUT,FALSE) == 0)//关ILL
		if(!SystemStatus.ILL)
		{
			if(l_tPwrCB.eMode != PM_MODE_WAIT_SLEEP)
			CANTask_SendPMReqest(OP_PM_STATE_WAIT_SLEEP);
    ...
    case CAN_PM_WAIT_SLEEP:
	  ...
	  */
		...
		break;
    case CAN_PM_SLEEP://关闭ACC之后,待机的时候的状态....
		/*
	 if(CanPMState.LocalACC == 1)
	  {
	  	CANTask_StartWork();
	 ...
	 else
	 {
		...
		if(IOStatus==2)
		{
			CANTask_StartWork();//将CAN唤醒...
			CANTask_SendPMReqest(OP_PM_STATE_ON);
			CanPMState.state = CAN_PM_HALFSLEEP;
			PanelLed_SwitchDeal(ON);//将POWER键点亮
	...
	break;
	case CAN_PM_HALFSLEEP://小灯唤醒整机的等待状态....
		if(CanPMState.LocalACC == 1)
		{
			CANTask_StartWork();
...
void CANTask_PMMsgHandler(tMSG_BUF* Msg)
{
	...
	/*
	//将经纬度信息,存储在no_init的数据块中,不写入到EEPROM中...
	//snake20161017 断电之前,进行EEPROM的写入...
	//只有在进入WAIT_SLEEP(两分钟后掉电)的时候,才写EEPROM..防止,异常掉电或者其他情况,发起了写请求,但是由于芯片断电导致,写错误...
...
static void CANTask_MsgHandler(tMSG_BUF* Msg)
{
    ...
    case MS_OTHER_AVM://snake20160808  盲区
    {
		CarAVMDataStruct *pData = pCAN_GetAVMData();

		/*
		if(!l_tMediaCB.uDevFlag.field.bUnLocked)//snake20160927 delete 同意之前也可以进入盲区...
		{
			return;
		}*/
		if(!HMIStatusFlag.g_tUpdataReverseParameter)//在倒车的时候,不响应盲区按键...
		if(1==CanPMState.SysWorkState)	
		{
			...
			if( Msg->iSid == OP_AVM_PRESS )
			{
				...
				{
					pData->BlindReq=1;
					_SendMsgToSRC(MS_INPUT_UI,UICC_STOP_SEEK,NULL);//snake20161007 进入盲区的时候停止收音机的搜索动作
				}				
			}
...
void CANTask_SetAmpPower(void)
{
	...
	if(CANTask_GetSysACCState() ==1)//acc  开
	{
	...
	if(CANTask_GetSysACCState() ==0)//acc 关
	{
...
void CANbus_Task(EVENT iEvt,eTASK_STATE eState)
{
    ...
    else if(eState == TASK_STATE_RUNNING)
    {
        ...
        if(iEvt & EVT_TASK_TICK_READY)
        {
...
#else
		#ifdef UDS_ENABLE_MACRO //snake20161012 关闭了网络管理,也要运行uds
	     ...
	     sCANModule.Ticks_Count++;
            ...
            if(sCANModule.Ticks_Count >=TIMER_1000MS)
            {
				...
				//snake20170418
				//临时增加,在没有找到真正的原因之前临时在这个每1S钟发起一次发送请求.
				CANUser_SendMsg500();
				CANUser_SendMsg501();
		...
		{
			if(++CanPmRecTimer >= 1250)	// 2ms*1250 = 2.5s 信号丢失2.5S，自动开机
			{
				CanPmRecTimer = 0;
